...
public abstract class BaseModel<M extends Model<M>> extends Model<M> {
	...
	public M findFirstByColumnValue(String columnName, Object columnValue,String columns) {
		List<M> result = findByColumnValue(columnName, columnValue, columns);
		if (result.isEmpty())
			return null;
		return result.get(0);
	}
	
	public M findFirstByColumnValue(String columnName, Object columnValue) {
		return findFirstByColumnValue(columnName,columnValue,"*");
	}
	
	public List<M> findByColumnValue(String columnName, Object columnValue) {
		return findByColumnValue(columnName, columnValue, "*");
	}
	
	/**
	 * ex：Map<String,Object> params = new hashMap<String,Object>();
	 * params.put("ids",1);
	 * params.put("name","zhangShang");
	 * findByColumnValue(params,"ids,name");
	 * 
	 * @param columnValueMap
	 * @param columns
	 * @return
	 */
	public List<M> findByColumnValue(Map<String,Object> columnValueMap) {
		return findByColumnValue(columnValueMap, "*");
	}
	/**
	 * ex：Map<String,Object> params = new hashMap<String,Object>();
	 * params.put("ids",1);
	 ...
	 * @return
	 */
	public List<M> findByColumnValue(Map<String,Object> columnValueMap, String columns) {
		Table table = getTable();
		...
		for (Map.Entry<String,Object> entry : columnValueMap.entrySet()) {
			...
			if (index < columnValueMap.size()-1){
				sql.append(" and ");
			}
			index++;
		...
		List<M> result = find(sql.toString(), params);
		return result;
	}
	
	public List<M> findByColumnValue(String columnName, Object columnValue, String columns) {
		Table table = getTable();
...
//			//System.out.println("Key = " + entry.getKey() + ", Value = " + entry.getValue());
//			sql.append("`").append(entry.getKey()).append("` = ?");
//			queryParams[index] = params.entrySet();
//			index++;
//		}
...
//		}
		...
		//sql.append(" and ");
		sql.append("`").append(columnName).append("` = ?");
		List<M> result = find(sql.toString(), columnValue);
		//return sql.toString();
